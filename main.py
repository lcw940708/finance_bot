...
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen flex flex-col justify-between">
    <header class="bg-white border-b border-slate-200 py-6">
        <div class="max-w-3xl mx-auto px-4">
            <h1 class="text-2xl font-bold text-slate-900">🔮 AI 廟街姓名學師傅</h1>
            <p class="text-xs text-slate-500 mt-1">地道香港師傅坐鎮，根據閣下姓名實時拆字與批算流年運勢。</p>
        </div>
    </header>
    ...
    <main class="max-w-3xl mx-auto px-4 py-8 flex-grow w-full">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4">
            <h2 class="text-lg font-bold text-slate-800">姓名學實時互動</h2>
            <p class="text-sm text-slate-600">隨時隨地入名同師傅傾偈，透過姓名能量為你解答各種疑難。</p>
            <div>
...
def generate_chat_page():
    """生成對話介面 fengshui_chat.html (支援強制姓名輸入與對話)"""
    chat_content = """<!DOCTYPE html>
<html lang="zh-HK">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>AI 廟街姓名學師傅對話</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen flex flex-col justify-between">
    ...
    <header class="bg-white border-b border-slate-200 py-6">
        <div class="max-w-3xl mx-auto px-4">
            ...
            <h1 class="text-2xl font-bold text-slate-900 mt-1">🔮 AI 廟街姓名學師傅</h1>
            <p class="text-xs text-slate-500 mt-1">請先輸入大名，師傅會先為你拆解姓名磁場，再解答你的提問。</p>
        </div>
    </header>
    ...
    <main class="max-w-3xl mx-auto px-4 py-6 flex-grow w-full space-y-4">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-xs flex items-center space-x-3">
            <label for="user-name" class="text-xs font-bold text-slate-700 whitespace-nowrap">請先輸入大名:</label>
        ...
        <div id="chat-container" class="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4 min-h-[350px] max-h-[500px] overflow-y-auto">
            <div class="flex items-start space-x-3">
                ...
                <div class="bg-slate-100 rounded-lg p-3 text-sm text-slate-700 max-w-[80%]">
                    「哈佬！坐低慢慢傾。喺上面留低個大名，然後隨便問事，師傅一定會先幫你拆解個名嘅玄機，再同你睇一睇！」
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-2">
            <input type="text" id="user-input" placeholder="例如：呢排財運點？或者問事業發展..." class="flex-grow border border-slate-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-amber-500">
    ...
    <script>
        async function sendQuestion() {
            ...
            if (!userName) {
                alert('請先在上方輸入你的大名，師傅先至好幫你拆字算命架！');
                nameField.focus();
                return;
            }
            ...
            chatContainer.innerHTML += `
                <div id="${loadingId}" class="flex items-start space-x-3">
                    <div class="bg-amber-600 text-white text-xs px-2.5 py-1 rounded-full font-bold">師傅</div>
                    <div class="bg-slate-100 rounded-lg p-3 text-sm text-slate-500 italic">師傅正透過姓名學天書為 ${escapeHtml(userName)} 拆解磁場與運勢...</div>
                </div>
            `;
            chatContainer.scrollTop = chatContainer.scrollHeight;
            ...
            try {
                ...
                chatContainer.innerHTML += `
                    <div class="flex items-start space-x-3">
                        <div class="bg-amber-600 text-white text-xs px-2.5 py-1 rounded-full font-bold">師傅</div>
                        <div class="bg-slate-100 rounded-lg p-3 text-sm text-slate-700 max-w-[80%]">${escapeHtml(data.content || '師傅一時靈感塞車，請再問一次。')}</div>
                    </div>
                `;
...
"""
    ...
    print("成功生成: fengshui_chat.html (強制輸入姓名版)")

if __name__ == "__main__":
    print("開始執行 AI 廟街姓名學師傅系統生成程序...")
