...
def scrape_horoscope_data():
    """透過穩定且免費的公開天文數據 API（香港日落日出時間）實時抓取天時數據"""
    os.makedirs("data", exist_ok=True)
    
    # 使用香港座標 (22.3193, 114.1694) 獲取今日實時日照與天象數據，絕不會 404
    target_url = "https://api.sunrise-sunset.org/json?lat=22.3193&lng=114.1694&formatted=0"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            scraped_content = {
                ...
                "celestial_data": data.get("results", {})
            }
        else:
    ...
    print("成功抓取實時天象數據並生成: data/fengshui.json")
...
<head>
    ...
    <title>Richard AI 實時天象危機預警站</title>
...
<body class="bg-slate-900 text-slate-100 font-sans min-h-screen flex flex-col justify-between">
    <header class="bg-slate-800 border-b border-rose-900/50 py-6">
        <div class="max-w-3xl mx-auto px-4">
            <h1 class="text-2xl font-bold text-rose-400">🌐 Richard AI 實時天象數據與危機預警站</h1>
            <p class="text-xs text-slate-400 mt-1">結合天文台實時日照、天象座標與玄學凶星進行精準預警。</p>
        </div>
    </header>
    ...
    <main class="max-w-3xl mx-auto px-4 py-8 flex-grow w-full">
        <div class="bg-slate-800 rounded-xl border border-rose-900/40 p-6 shadow-lg space-y-4">
            <h2 class="text-lg font-bold text-rose-300">實時數據同步與批算</h2>
            <p class="text-sm text-slate-300">後台透過穩定的開源天文 API 獲取今日香港日照與天象數據，由 Richard AI 為你解析危機！</p>
            <div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Richard AI 實時天象批算系統</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-900 text-slate-100 font-sans min-h-screen flex flex-col justify-between">
    ...
    <header class="bg-slate-800 border-b border-rose-900/50 py-6">
        <div class="max-w-3xl mx-auto px-4 flex justify-between items-center">
            <div>
                ...
                <h1 class="text-2xl font-bold text-rose-400 mt-1">🌐 Richard AI 實時天象運程批算</h1>
                <p class="text-xs text-slate-400 mt-1">結合香港實時日照天象與Richard級香港廣東話剖析。</p>
            </div>
    ...
    <main class="max-w-3xl mx-auto px-4 py-6 flex-grow w-full space-y-4">
        <div class="bg-slate-800 rounded-xl border border-rose-900/40 p-6 shadow-lg space-y-4">
            ...
            <div>
                <button onclick="getFortune()" id="send-btn" class="w-full bg-rose-600 hover:bg-rose-700 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition shadow-md">請 Richard AI 結合實時天象批算</button>
            </div>
        </div>
        ...
        <div id="chat-container" class="bg-slate-800 rounded-xl border border-rose-900/40 p-6 shadow-lg space-y-4 min-h-[350px] max-h-[500px] overflow-y-auto">
            <div class="flex items-start space-x-3">
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-3 text-sm text-slate-200 max-w-[80%]">
                    「老實講，後台已經透過香港實時天象座標捕捉咗今日嘅日照與陽氣轉折點。填好你個名同生日，本Richard即刻同你拆解！」
                </div>
            </div>
        </div>
    </main>
        ...
        async function getFortune() {
            ...
            chatContainer.innerHTML += `
                <div class="flex items-start justify-end space-x-3">
                    <div class="bg-slate-700 text-slate-100 rounded-lg p-3 text-sm max-w-[80%]"><b>[${escapeHtml(userName)} | 生日: ${escapeHtml(userDob)} | 星座: ${exactZodiac}]</b> 請 Richard AI Richard批算</div>
            ...
            sendBtn.innerText = 'Richard正結合實時天象分析中...';
            ...
            chatContainer.innerHTML += `
                <div id="${loadingId}" class="flex items-start space-x-3">
                    <div class="bg-rose-600 text-white text-xs px-2.5 py-1 rounded-full font-bold">Richard AI</div>
                    <div class="bg-slate-900 border border-slate-700 rounded-lg p-3 text-sm text-slate-400 italic">正在剖析今日實時天象數據，為「${escapeHtml(userName)}」（${exactZodiac}）進行深度批算...</div>
                </div>
            `;
            chatContainer.scrollTop = chatContainer.scrollHeight;
            ...
            try {
                const response = await fetch('https://mainbot.lcw940708.workers.dev/', {
                    ...
                    body: JSON.stringify({ 
                        ...
                        prompt: "請結合實時天象數據與精確星座，嚴厲指出今日要小心咩陷阱、最旺數字同開運花卉。" 
                    })
                });
            ...
            sendBtn.innerText = '請 Richard AI 結合實時天象批算';
            nameField.focus();
            chatContainer.scrollTop = chatContainer.scrollHeight;
        }
...
"""
    ...
    print("成功生成: fengshui_chat.html (實時天象數據整合版)")

if __name__ == "__main__":
    print("開始執行 Richard AI 實時天象數據抓取與網頁生成程序...")
