...
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen flex flex-col justify-between">
    <header class="bg-white border-b border-slate-200 py-6">
        <div class="max-w-3xl mx-auto px-4">
            ...
            <p class="text-xs text-slate-500 mt-1">Richard AI坐鎮，輸入大名與出生日期，即時為你進行八字姓名學批算、幸運數字與開運花卉建議。</p>
        </div>
    </header>
    ...
    <main class="max-w-3xl mx-auto px-4 py-8 flex-grow w-full">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4">
            <h2 class="text-lg font-bold text-slate-800">生辰姓名全方位運勢批算</h2>
            <p class="text-sm text-slate-600">結合姓名磁場與出生年月日，Richard AI 即刻為你開壇解構今日運勢、最旺數字同開運花卉！</p>
            <div>
                <a href="fengshui_chat.html" class="inline-block bg-amber-600 hover:bg-amber-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition">立即讓 Richard AI 批算</a>
            </div>
        </div>
    </main>
...
def generate_chat_page():
    """生成輸入姓名與出生日期、顯示即時日期時間的 fengshui_chat.html"""
    chat_content = """<!DOCTYPE html>
<html lang="zh-HK">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Richard AI 生辰姓名深度批算</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 text-slate-800 font-sans min-h-screen flex flex-col justify-between">
    ...
    <header class="bg-white border-b border-slate-200 py-6">
        <div class="max-w-3xl mx-auto px-4 flex justify-between items-center">
            <div>
                ...
                <h1 class="text-2xl font-bold text-slate-900 mt-1">🔮 Richard AI 生辰姓名深度批算</h1>
                <p class="text-xs text-slate-500 mt-1">報上大名與生辰，解構姓名磁場、今日最旺數字及開運花卉。</p>
            </div>
            <!-- 即時日期時間顯示 -->
        ...
        <div id="chat-container" class="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4 min-h-[350px] max-h-[500px] overflow-y-auto">
            <div class="flex items-start space-x-3">
                ...
                <div class="bg-slate-100 rounded-lg p-3 text-sm text-slate-700 max-w-[80%]">
                    「哈佬！坐低慢慢嘆杯茶。請喺上面填好你個大名同埋出生日期，Richard AI 即刻結合今日嘅時辰八字，同你睇睇姓名磁場、今日最旺你嘅數字，同埋最適合拎住或者擺放咩類型嘅開運花卉！」
                </div>
            </div>
        </div>
    </main>
        ...
        async function getFortune() {
            ...
            chatContainer.innerHTML += `
                <div class="flex items-start justify-end space-x-3">
                    <div class="bg-amber-50 text-slate-800 rounded-lg p-3 text-sm max-w-[80%]"><b>[${escapeHtml(userName)} | 出生: ${escapeHtml(userDob)}]</b> 請 Richard AI 批算（批算時間：${currentDateTimeStr}）</div>
            ...
            sendBtn.innerText = 'Richard AI推算緊八字與開運建議...';
            ...
            chatContainer.innerHTML += `
                <div id="${loadingId}" class="flex items-start space-x-3">
                    <div class="bg-amber-600 text-white text-xs px-2.5 py-1 rounded-full font-bold">Richard AI</div>
                    <div class="bg-slate-100 rounded-lg p-3 text-sm text-slate-500 italic">Richard AI 正結合今日（${currentDateTimeStr}）天時、生辰八字與姓名磁場為「${escapeHtml(userName)}」推算中...</div>
                </div>
            `;
            chatContainer.scrollTop = chatContainer.scrollHeight;
            ...
            try {
                ...
                const response = await fetch('https://mainbot.lcw940708.workers.dev/', {
                    ...
                    body: JSON.stringify({ 
                        ...
                        prompt: "請結合我的姓名與出生日期，給予全面運勢批算，並必須列出今日最旺我的數字及最適合帶備/擺放的開運花卉類型。" 
                    })
                });
                ...
                chatContainer.innerHTML += `
                    <div class="flex items-start space-x-3">
                        <div class="bg-amber-600 text-white text-xs px-2.5 py-1 rounded-full font-bold">Richard AI</div>
                        <div class="bg-slate-100 rounded-lg p-3 text-sm text-slate-700 max-w-[80%]" style="white-space: pre-line;">${escapeHtml(data.content || 'Richard AI一時靈感塞車，請稍後重試。')}</div>
                    </div>
                `;
...
"""
    ...
    print("成功生成: fengshui_chat.html (生辰姓名 + 即時時間 + 幸運數字/開運花卉版)")

if __name__ == "__main__":
    print("開始執行 Richard AI 生辰姓名批算系統生成程序...")
